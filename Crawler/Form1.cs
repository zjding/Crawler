...
namespace Crawler
{
    public partial class Form1 : Form
    {
        ...
        private void GetProductInfo(bool bTruncate = true, bool bTruncateCostcoCategoriesTable = true)
        {
            ...
            foreach (string pu in productUrlArray)
            {
                try
                {
                    ...
                    // feature
                    string shipping = "0";
                    if (hasElement(eProductDetails, By.ClassName("features-container")))
                    {
                        IWebElement eFeatures = eProductDetails.FindElement(By.ClassName("features-container"));

                        if (hasElement(eFeatures, By.Id("shipping-statement")))
                        {
                    ...
                    sqlString = "INSERT INTO Raw_ProductInfo (Name, UrlNumber, ItemNumber, Category, Price, Shipping, Discount, ImageLink, ImageOptions, Url, Options, Thumb) VALUES ('" + 
                        productName.Replace("'", "''") + "','" + UrlNum + "','" + itemNumber + "','" + stSubCategories + "'," + price + "," + shipping + "," + "'" + discount + 
                        "','" + imageLink.Replace("'", "''") + "','" + imageOptions.Replace("'", "''") + "','" + productUrl.Replace("'", "''") + "','" + optionsString + "','" + thumb + "')";
                    cmd.CommandText = sqlString;
                    cmd.ExecuteNonQuery();
                    nImportProducts++;
        ...
        private void PopulateTables()
        {
            ...
            sqlString = @"insert into dbo.staging_productInfo (Name, urlNumber, itemnumber, Category, price, shipping, discount, details, specification, imageLink, imageOptions, url, options, Thumb)
                        select distinct Name, urlNumber, itemnumber, Category, price, shipping, discount, details, specification, imageLink, imageOptions, url, options, Thumb
        ...
        private void CompareCostcoInventory()
        {
            ...
            sqlString = @"  INSERT INTO CostcoInventoryChange_PriceUp (Name, CostcoUrl, UrlNumber, CostcoOldPrice, CostcoNewPrice, ImageLink, Thumb) 
                            SELECT s.Name, s.Url, s.UrlNumber, p.Price, s.Price, s.ImageLink, s.Thumb
                            FROM [dbo].[Staging_ProductInfo] s, [dbo].[ProductInfo] p
                            WHERE s.UrlNumber = p.UrlNumber
                            AND s.Price > p.Price ";
            ...
            sqlString = @"  INSERT INTO CostcoInventoryChange_PriceDown (Name, CostcoUrl, UrlNumber, CostcoOldPrice, CostcoNewPrice, ImageLink, Thumb) 
                            SELECT s.Name, s.Url, s.UrlNumber, p.Price, s.Price, s.ImageLink, s.Thumb
                            FROM [dbo].[Staging_ProductInfo] s, [dbo].[ProductInfo] p
                            WHERE s.UrlNumber = p.UrlNumber
                            AND s.Price < p.Price ";
            ...
            sqlString = @"  INSERT INTO CostcoInventoryChange_Discontinue (Name, CostcoUrl, UrlNumber, Thumb)
                            SELECT p.Name, p.Url, p.UrlNumber, p.Thumb
                            FROM ProductInfo p 
                            WHERE 
                            NOT EXISTS
                            (SELECT 1 FROM Staging_ProductInfo sp WHERE sp.UrlNumber = p.UrlNumber)";
        ...
        private void CompareEBayListings(out int nEBayListingChangePriceUp, out int nEBayListingChangePriceDown, out int nEBayListingChangeDiscontinue, out int nEBayListingChangeOptions)
        {
            ...
            sqlString = @"INSERT INTO [dbo].[eBayListingChange_PriceUp] (Name, CostcoUrl, UrlNumber, eBayItemNumber, CostcoOldPrice, CostcoNewPrice, eBayListingOldPrice, ImageLink, Thumb)
                                select p.name as Name, p.Url as CostcoUrl, p.UrlNumber as UrlNumber, l.eBayItemNumber as eBayItemNumber, p.Price as CostcoOldPrice, s.Price as CostcoNewPrice, l.eBayListingPrice as eBayListingOldPrice, p.ImageLink as ImageLink, s.Thumb as Thumb
            ...
            sqlString = @"  UPDATE productInfo  
                            SET price = u.CostcoNewPrice
                            FROM productInfo p, eBayListingChange_PriceUp u
                            WHERE p.UrlNumber = u.UrlNumber";
            cmd.CommandText = sqlString;
            cmd.ExecuteNonQuery();


            // eBay listing price down
            sqlString = @"INSERT INTO [dbo].[eBayListingChange_PriceDown] (Name, CostcoUrl, UrlNumber, eBayItemNumber, CostcoOldPrice, CostcoNewPrice, eBayListingOldPrice, ImageLink, Thumb)
                                select p.name as Name, p.Url as CostcoUrl, p.UrlNumber as UrlNumber, l.eBayItemNumber as eBayItemNumber, p.Price as CostcoOldPrice, s.Price as CostcoNewPrice, l.eBayListingPrice as eBayListingOldPrice, p.ImageLink as ImageLink, s.Thumb as Thumb
                                from [dbo].[Staging_ProductInfo] s, [dbo].[ProductInfo] p, [dbo].[eBay_CurrentListings] l
                                where s.UrlNumber = p.UrlNumber
                                and s.UrlNumber = l.CostcoUrlNumber
            ...
            sqlString = @"  UPDATE productInfo  
                            SET price = u.CostcoNewPrice
                            FROM productInfo p, eBayListingChange_PriceDown u
                            WHERE p.UrlNumber = u.UrlNumber";
            cmd.CommandText = sqlString;
            cmd.ExecuteNonQuery();
            ...
            sqlString = @"  DELETE FROM productInfo
                            WHERE UrlNumber in (SELECT UrlNumber FROM eBayListingChange_Discontinue)";
            cmd.CommandText = sqlString;
            cmd.ExecuteNonQuery();

           
            // options change 
            sqlString = @"INSERT INTO [dbo].[eBayListingChange_OptionChange] (Name, CostcoUrl, UrlNumber, eBayItemNumber, CostcoOldOptions, CostcoNewOptions, CostcoNewImageOptions, ImageLink, Thumb)
                        select s.name as Name, s.Url as CostcoUrl, s.UrlNumber as UrlNumber, l.eBayItemNumber as eBayItemNumber, p.Options as CostcoOldOptions, s.Options as CostcoNewOptions, s.ImageOptions as CostcoNewImageOptions, p.ImageLink as ImageLink, s.Thumb as Thumb
            ...
            sqlString = @"  UPDATE productInfo  SET Options = u.CostcoNewOptions, ImageOptions = u.CostcoNewImageOptions
                            FROM productInfo p, eBayListingChange_OptionChange u
                            WHERE p.UrlNumber = u.UrlNumber";
            cmd.CommandText = sqlString;
            cmd.ExecuteNonQuery();
